...
static void log_option(const char *pfx, const uint8_t *opt)
{
	if (dhcp_verbose >= 2) {
		...
		*bin2hex(buf, (void*) (opt + OPT_DATA), opt[OPT_LEN]) = '\0';
		UDHCP_DEBUG("%s: 0x%02x %s", pfx, opt[OPT_CODE], buf);
	}
}
#else
...
#endif
#if 0
unsigned ICACHE_FLASH_ATTR udhcp_option_idx(const char *name)
{
...

	{
		...
		*d = '\0';
		UDHCP_DEBUG("unknown option '%s', known options: %s", name, buf);
	}
}
#endif
/* Get an option with bounds checking (warning, result is not aligned) */
uint8_t* ICACHE_FLASH_ATTR udhcp_get_option(struct dhcp_packet *packet, int code)
{
	uint8_t *optionptr;
	int len;
	int rem;
	...
	while (1) {
		if (rem <= 0) {
			UDHCP_DEBUG("bad packet, malformed option field\n");
			return NULL;
		}
		...
		if (optionptr[OPT_CODE] == code) {
			UDHCP_DEBUG("Option found 0x%02x\n", optionptr[OPT_CODE]);
	...
	UDHCP_DEBUG("Option 0x%02x not found\n", code);
	return NULL;
}

/* Return the position of the 'end' option (no bounds checking) */
int ICACHE_FLASH_ATTR udhcp_end_option(uint8_t *optionptr)
{
	int i = 0;
 ...
 */
void ICACHE_FLASH_ATTR udhcp_add_binary_option(struct dhcp_packet *packet, uint8_t *addopt)
{
	unsigned len;
...
//TODO: learn how to use overflow option if we exhaust packet->options[]
		UDHCP_DEBUG("option 0x%02x did not fit into the packet",
				addopt[OPT_CODE]);
		return;
	}
...
void ICACHE_FLASH_ATTR udhcp_add_simple_option(struct dhcp_packet *packet, uint8_t code, uint32_t data)
{
	...
	UDHCP_DEBUG("can't add option 0x%02x", code);
}

/* Find option 'code' in opt_list */
struct option_set* ICACHE_FLASH_ATTR udhcp_find_option(struct option_set *opt_list, uint8_t code)
{
...
int ICACHE_FLASH_ATTR udhcp_str2nip(const char *str, void *arg)
{
	ip_addr_t lsa;
...
static char *ICACHE_FLASH_ATTR allocate_tempopt_if_needed(
...
//		end = hex2bin(allocated, buffer, 255);
		if (errno)
			UDHCP_DEBUG("malformed hex string '%s'", buffer);
		*length_p = end - allocated;
	}
	return allocated;
}
/* helper: add an option to the opt_list */
static void ICACHE_FLASH_ATTR attach_option(
		...
		int length)
{
	...
	if (!existing) {
		...
		/* make a new option */
		UDHCP_DEBUG("Attaching option %02x to list", optflag->code);
		new = (struct option_set *)zalloc(sizeof(*new));
		new->data = (uint8_t *)zalloc(length + OPT_DATA);
	...
	if (optflag->flags & OPTION_LIST) {
		...
		UDHCP_DEBUG("Attaching option %02x to existing member of list", optflag->code);
		old_len = existing->data[OPT_LEN];
		if (old_len + length < 255) {
			/* actually 255 is ok too, but adding a space can overlow it */

			existing->data = (uint8_t *)realloc(existing->data, OPT_DATA + 1 + old_len + length);
...
int ICACHE_FLASH_ATTR udhcp_str2optset(const char *const_str, void *arg)
{
...
//		}
		...
		case OPTION_S32: {
			sint32_t tmp = bb_strtoi32(val, NULL, 0);
			*result_u32 = htonl(tmp);
			retval = (errno == 0);
			break;
		}
...
int ICACHE_FLASH_ATTR sprint_nip6(char *dest, /*const char *pre,*/ const uint8_t *ip)
{
	char hexstrbuf[16 * 2];
