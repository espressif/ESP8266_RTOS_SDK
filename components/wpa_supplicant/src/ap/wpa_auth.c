...
static const u32 dot11RSNAConfigGroupUpdateCount ICACHE_RODATA_ATTR = 4;
static const u32 dot11RSNAConfigPairwiseUpdateCount ICACHE_RODATA_ATTR = 4;
static const u32 eapol_key_timeout_first ICACHE_RODATA_ATTR = 100; /* ms */
static const u32 eapol_key_timeout_subseq ICACHE_RODATA_ATTR = 1000; /* ms */
static const u32 eapol_key_timeout_first_group ICACHE_RODATA_ATTR = 500; /* ms */

#define WPA_SM_MAX_INDEX 16
static void* s_sm_table[WPA_SM_MAX_INDEX];
static u32 s_sm_valid_bitmap = 0;

os_timer_t resend_eapol;
...
static struct ICACHE_FLASH_ATTR wpa_state_machine* wpa_auth_get_sm(u32 index)
{
...
static void ICACHE_FLASH_ATTR wpa_auth_add_sm(struct wpa_state_machine* sm)
{
    if (sm) {
        u8 i;
...
static void ICACHE_FLASH_ATTR wpa_auth_del_sm(struct wpa_state_machine* sm)
{
...
static inline int ICACHE_FLASH_ATTR wpa_auth_mic_failure_report(
...
static inline void ICACHE_FLASH_ATTR wpa_auth_set_eapol(struct wpa_authenticator* wpa_auth,
...
static inline int ICACHE_FLASH_ATTR wpa_auth_get_eapol(struct wpa_authenticator* wpa_auth,
        const u8* addr, wpa_eapol_variable var)
{
    return -1;
}

static inline const u8* ICACHE_FLASH_ATTR wpa_auth_get_psk(struct wpa_authenticator* wpa_auth,
...
static inline int ICACHE_FLASH_ATTR wpa_auth_get_msk(struct wpa_authenticator* wpa_auth,
        const u8* addr, u8* msk, size_t* len)
{
    return -1;
}

static inline int ICACHE_FLASH_ATTR wpa_auth_set_key(struct wpa_authenticator* wpa_auth,
        int vlan_id,
...
static inline int ICACHE_FLASH_ATTR wpa_auth_get_seqnum(struct wpa_authenticator* wpa_auth,
...
static int ICACHE_FLASH_ATTR wpa_auth_send_cb(esp_aio_t* aio)
{
...
int ICACHE_FLASH_ATTR wpa_auth_for_each_sta(struct wpa_authenticator* wpa_auth,
...
static void ICACHE_FLASH_ATTR wpa_sta_disconnect(struct wpa_authenticator* wpa_auth,
        const u8* addr)
{
...
static int ICACHE_FLASH_ATTR wpa_use_aes_cmac(struct wpa_state_machine* sm)
{
    int ret = 0;
...
static void ICACHE_FLASH_ATTR wpa_rekey_gtk(void* eloop_ctx, void* timeout_ctx)
{
    struct wpa_authenticator* wpa_auth = eloop_ctx;
    struct wpa_group* group;
...
static void ICACHE_FLASH_ATTR wpa_rekey_ptk(void* eloop_ctx, void* timeout_ctx)
{
    struct wpa_authenticator* wpa_auth = eloop_ctx;
...
static int ICACHE_FLASH_ATTR wpa_group_init_gmk_and_counter(struct wpa_authenticator* wpa_auth,
        struct wpa_group* group)
{
...
static struct wpa_group* ICACHE_FLASH_ATTR wpa_group_init(struct wpa_authenticator* wpa_auth,
 ...
 */
struct wpa_authenticator* ICACHE_FLASH_ATTR wpa_init(const u8* addr,
...
struct wpa_state_machine* ICACHE_FLASH_ATTR
...
int ICACHE_FLASH_ATTR wpa_auth_sta_associated(struct wpa_authenticator* wpa_auth,
        struct wpa_state_machine* sm)
{
...
void ICACHE_FLASH_ATTR wpa_auth_sta_no_wpa(struct wpa_state_machine* sm)
{
...
static void ICACHE_FLASH_ATTR wpa_free_sta_sm(struct wpa_state_machine* sm)
{
    wpa_auth_del_sm(sm);
...
void ICACHE_FLASH_ATTR wpa_auth_sta_deinit(struct wpa_state_machine* sm)
{
    if (sm == NULL) {
        return;
    }
...
static void ICACHE_FLASH_ATTR wpa_request_new_ptk(struct wpa_state_machine* sm)
{
    if (sm == NULL) {
        return;
    }
...
static int ICACHE_FLASH_ATTR wpa_replay_counter_valid(struct wpa_key_replay_counter* ctr,
...
static void ICACHE_FLASH_ATTR wpa_replay_counter_mark_invalid(struct wpa_key_replay_counter* ctr,
        const u8* replay_counter)
{
    int i;
...
static int ICACHE_FLASH_ATTR wpa_receive_error_report(struct wpa_authenticator* wpa_auth,
...
void ICACHE_FLASH_ATTR wpa_receive(struct wpa_authenticator* wpa_auth,
                                   struct wpa_state_machine* sm,
...
static int ICACHE_FLASH_ATTR wpa_gmk_to_gtk(const u8* gmk, const char* label, const u8* addr,
...
static void ICACHE_FLASH_ATTR wpa_send_eapol_timeout(void* eloop_ctx, void* timeout_ctx)
{
    struct wpa_authenticator* wpa_auth = eloop_ctx;
    struct wpa_state_machine* sm = timeout_ctx;
...
void ICACHE_FLASH_ATTR __wpa_send_eapol(struct wpa_authenticator* wpa_auth,
...
void ICACHE_FLASH_ATTR resend_eapol_handle(void* timeout_ctx)
{
...
static void ICACHE_FLASH_ATTR wpa_send_eapol(struct wpa_authenticator* wpa_auth,
        struct wpa_state_machine* sm, int key_info,
        const u8* key_rsc, const u8* nonce,
        const u8* kde, size_t kde_len,
        int keyidx, int encr)
{
    ...
    os_timer_disarm(&resend_eapol);
    os_timer_setfn(&resend_eapol, (os_timer_func_t*)resend_eapol_handle, (void*)(sm->index));
    os_timer_arm(&resend_eapol, 1000, 0);
}


static int ICACHE_FLASH_ATTR wpa_verify_key_mic(struct wpa_ptk* PTK, u8* data, size_t data_len)
{
    struct ieee802_1x_hdr* hdr;
    struct wpa_eapol_key* key;
    u16 key_info;
...
void ICACHE_FLASH_ATTR wpa_remove_ptk(struct wpa_state_machine* sm)
{
    sm->PTK_valid = FALSE;
...
int ICACHE_FLASH_ATTR wpa_auth_sm_event(struct wpa_state_machine* sm, wpa_event event)
{
    int remove_ptk = 1;
...
static void ICACHE_FLASH_ATTR wpa_group_ensure_init(struct wpa_authenticator* wpa_auth,
        struct wpa_group* group)
{
...
static int ICACHE_FLASH_ATTR wpa_derive_ptk(struct wpa_state_machine* sm, const u8* pmk,
        struct wpa_ptk* ptk)
{
...
static int ICACHE_FLASH_ATTR ieee80211w_kde_len(struct wpa_state_machine* sm)
{
    return 0;
}


static u8* ICACHE_FLASH_ATTR ieee80211w_kde_add(struct wpa_state_machine* sm, u8* pos)
{
    return pos;
}
...
#endif /* CONFIG_IEEE80211R */
    kde = (u8*)wpa_malloc_dram(kde_len);
...
static int ICACHE_FLASH_ATTR wpa_gtk_update(struct wpa_authenticator* wpa_auth,
        struct wpa_group* group)
{
    int ret = 0;
...
static void ICACHE_FLASH_ATTR wpa_group_gtk_init(struct wpa_authenticator* wpa_auth,
        struct wpa_group* group)
{
...
static int ICACHE_FLASH_ATTR wpa_group_update_sta(struct wpa_state_machine* sm, void* ctx)
{
...
static void ICACHE_FLASH_ATTR wpa_group_setkeys(struct wpa_authenticator* wpa_auth,
        struct wpa_group* group)
{
    int tmp;
...
static int ICACHE_FLASH_ATTR wpa_group_config_group_keys(struct wpa_authenticator* wpa_auth,
        struct wpa_group* group)
{
    int ret = 0;
    ...
    if (wpa_auth_set_key(wpa_auth, group->vlan_id,
                         ...
                         (u8*)broadcast_ether_addr, group->GN,
                         group->GTK[group->GN - 1], group->GTK_len) < 0) {
        ret = -1;
    }
...
static int ICACHE_FLASH_ATTR wpa_group_setkeysdone(struct wpa_authenticator* wpa_auth,
        struct wpa_group* group)
{
    wpa_printf(MSG_DEBUG, "WPA: group state machine entering state "
...
static void ICACHE_FLASH_ATTR wpa_group_sm_step(struct wpa_authenticator* wpa_auth,
        struct wpa_group* group)
{
    if (group->GInit) {
...
static int ICACHE_FLASH_ATTR wpa_sm_step(struct wpa_state_machine* sm)
{
    if (sm == NULL) {
        return 0;
    }
