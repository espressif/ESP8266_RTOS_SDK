                                     ...
                                     const httpd_uri_t *uri_handler)
{
    ...
    if (httpd_find_uri_handler(handle, uri_handler->uri, strlen(uri_handler->uri), uri_handler->method, &err) != NULL) {
        ESP_LOGW(TAG, LOG_FMT("handler %s with method %d already registered err: %i"),
                 uri_handler->uri, uri_handler->method, err);
