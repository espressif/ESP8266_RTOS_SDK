...
def sizes_overall(sections):
    ...
    result = {}
    for section in sections.values():
        result[section["name"]] = 0
        for s in section["sources"]:
            result[section["name"]] += s["size"]
    return result
...
def print_summary(memory_config, sections):
    ...
    total_flash = memory_config["irom0_0_seg"]["length"]

    # Snippet from one of datasheets:
    #   "
    ...
    # dram:
    #       .data .bss .roadata and heap
    #       -
    # iram:
    #       .text .lit4
    #       -functions intended for fast execution go here
    # irom: 
    #       irom0.text      
    #       -external flash. Most of code goes here
    #
    # Total size of RAM == 160 kB
    # 96 kB goes for dram
    # 64 kB goes for (iram + cache)
    #   cache is used to copy function from external flash into, before function execution
    # some info avaliable at components/esp8266/ld/esp8266.ld and components/esp8266/ld/esp8266.common.ld

    # some kind of summary form esp8266.common.ld
    ...
    #     if not section.startswith(".debug"):
    #         print("Section: %20s: %s" % (section, val["size"]))

    print("Summary: ")

    used_data = get_size(".data")
    used_bss = get_size(".bss")
    used_rodata = get_size(".rodata")
    used_dram = used_data + used_bss + used_rodata

    print("Used static DRAM: %7d bytes (%7d available, %.1f%% used) Total: %3d kB" %
          (used_dram, total_dram - used_dram,
           100.0 * used_dram / total_dram, total_dram / 1024))
    print("    DRAM .data    size: %7d bytes" % used_data)
    print("    DRAM .bss     size: %7d bytes" % used_bss)
    print("    DRAM .rodata  size: %7d bytes" % used_rodata)


    used_iram = get_size(".text")
    print("Used static IRAM: %7d bytes (%7d available, %.1f%% used) Total: %3d kB" %
          (used_iram, total_iram - used_iram,
           100.0 * used_iram / total_iram, total_iram / 1024))
    print("    Cache size (IRAM + cache = 64 kB. Distribution can be modified):   %3d kB" % (64 -total_iram / 1024))

    flash_code = get_size(".irom0.text")

    print("Used Flash: %7d bytes (%7d available, %.1f%% used)       Total: %3d kB" %
          (flash_code, total_flash - flash_code,
           100.0 * flash_code / total_flash, total_flash / 1024))
...
def print_detailed_sizes(sections, key, header):
    ...
    for k in sizes:
        ...
        result[k]["iram"] = v.get(".text",0)
        result[k]["flash_text"] = v.get(".irom0.text", 0)
...
def print_archive_symbols(sections, archive):
    interested_sections = [".data", ".bss", ".text", ".vectors", ".irom0.text", ".rodata"]
    result = {}
